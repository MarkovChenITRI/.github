...
1. **正文先成立，圖表後補**：正文必須先把概念、判斷、因果、邊界與讀者要採取的理解路徑講清楚；圖表只能做壓縮、索引與佐證，不得取代正文承擔論證。
2. **首次交互參照必須先於圖表出現**：任何「如圖 1」「如表 2」「詳見表 3」或等價參照，都必須先在正文段落中出現，圖表本體才可以往下出現；不得先放圖表，再於後文第一次說明它的用途。
3. **圖表只承載穩定事實，不承載推論**：可放入圖表者，限於欄位名、分類、來源位置、驗證面向、條件類型等穩定資訊；凡是理由、用途、後果、風險判讀、處置建議、下一步、代表什麼問題等推論型內容，一律回正文。
4. **Caption 只命名，不論證**：caption 只能交代圖表名稱與識別，不得補正文未先說清楚的論點、用途或判斷。
5. **圖表前必須有導入段**：每張圖表出現前，正文至少要先交代這張圖表支援哪個論點、讀者看它要看什麼，以及它不能取代哪一段正文。
6. **圖表存在的理由是壓縮，不是補洞**：若移除圖表，正文仍應成立；圖表只能讓讀者更快核對，不得用來彌補正文沒寫清楚的邏輯斷層。
...
9. **圖表後不得第一次定義圖表用途**：圖表後可以有 caption 或閱讀提醒，但不得在圖表出現後才第一次解釋「這張圖其實在做什麼」。
10. **表格不是流程，流程不是表格**：若內容本質上是判斷順序、失敗分類、處置建議或責任分流，優先回正文或步驟段落，不得為了整齊而硬壓成表格。
