...
description: "Documentation Experience Manager playbook. Use when creating README, onboarding, quickstart, system docs, developer handoff docs, documentation acceptance criteria, multi-page doc rewrites, documentation IA changes, reader-path redesign, or terminology resets."
...
只要同時改動多頁、重設頁名、改變讀者路徑、重整資訊架構或重命名共用名詞，就必須先套用本 playbook，再開始正式出稿。本 skill 是 workflow checklist，不是獨立 owner。
...
正式改寫多頁人類文件前，先確認下列工件都存在；缺任一項時，只能產出 `draft-placeholder`、`deferred` 或等價草稿：
...
- 每一頁都要寫清楚處理物件、輸入訊號、操作位置、完成訊號與下一位行動者。
- 每個條件分支、例外情境與退回路徑都要寫明 owner，不留「請確認」這類無主詞指示。
- 同一個資料物件、流程段、狀態、角色或交付物，在同一個 scope 內只用一套主名稱。
- API 名稱、欄位名、enum 與程式字面值可保留原文，但第一次出現時要補白話名稱與觀察位置。

## 收尾驗收

- 重讀所有受影響頁、同一讀者路徑的相鄰頁、pointer 頁與 reference 頁。
- 執行 reader-path audit、actor trace、terminology sweep、cross-link audience audit 與 cold-read review。
- 確認 build、link check、grep、lint 只被當成必要條件，而不是文件完成證明。
- 確認未驗證內容仍標示為待確認，沒有被包裝成既成事實。
