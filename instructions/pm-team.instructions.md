...
4. 指定這一頁是任務型、概念型、參考型，還是 prose-first 的設計／維運規格頁，避免 owner 把不同頁型混寫。
5. 凍結圖表職責：圖表是索引、壓縮還是證據，不得放任 owner 用圖表承擔正文論證。
...
3. 最低成功標準是什麼。
4. 哪些內容不在本次交付範圍。
5. 這頁的主體應以正文、步驟還是參考表為主。
6. 哪些圖表若存在，只能作為索引或佐證，不得成為第一理解入口。

若缺這些項目，RD / QE / 當前任務 owner 不得自行腦補成最終對外表達。
...
7. `Figure / Table Contract`：哪些圖表可以存在、各自支援哪個論點、不得承擔哪些正文責任。

若缺上述任一工件，或 PM / RD / QE 來源尚未齊備，當前任務 owner 只能產出 `draft-placeholder`、`deferred` 或等價草稿，不得寫成定稿事實，也不得宣稱該 section 已完成。

### 期刊式圖表責任

當文件屬設計規格、維運機制、責任邊界、流程說明或其他 prose-first 頁面時，PM 必須主動限制圖表職責：

1. 不得要求 owner 以圖表取代正文主體。
2. 不得把「為什麼」「風險」「是否應拒發」「下一步怎麼做」這類推論內容指定成表格欄位。
3. 每張圖表都必須先有正文導入段，交互參照先於圖表本體出現。
4. 若頁面讀者只看圖表就會誤解主張，PM 必須要求 owner 把主論證補回正文。
