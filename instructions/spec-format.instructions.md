...
整個專案固定一份 `<專案資料夾>/.github/specs/overview.md`，記錄這個專案的系統架構。系統架構採用領域驅動設計（DDD）分層，固定四層：使用者介面層、應用層、領域層、基礎設施層。撰寫模板見 `.github/templates/overview.md`。
...
不管哪一種目錄結構，每個模組固定四個欄位（Who、What、How、Where），撰寫模板見 `.github/templates/module-spec.md`。
...
3. **選一張圖，圖後接讀圖指引**：依 `plan.instructions.md`「圖表類型的選用規則」，依這個模組的內容性質挑合適的圖表類型，圖後接一段「讀這張圖時，先看什麼、再看什麼」的讀圖指引。
...
整個專案的每一條使用者操作流程，固定一份 `<專案資料夾>/.github/specs/usage-examples/<flow-name>.md`，記錄這條流程的驗收標準。撰寫模板見 `.github/templates/usage-example.md`。
