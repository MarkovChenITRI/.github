...
交文件時，RD 提供的是**穩定事實包**，不是直接代寫最終正文。凡是「為什麼這樣設計」「失敗時表示什麼」「維護者下一步怎麼做」這類推論，必須與穩定事實分開標記，避免當前任務 owner 把推論直接塞進表格欄位或 caption。
...
當 RD 提供會進入 README、維運文件、規格頁或概念頁的資料表時：

1. 只提供穩定欄位：欄位名、分類、來源位置、驗證面向、條件類型、依賴關係。
2. 不把理由、後果、風險判讀、處置建議寫成預設表格欄位。
3. 若某資訊必須靠句子推理才成立，應明確標成正文候選內容，而不是表格欄位。
4. 不把流程順序圖當成唯一規格交付物；圖前仍需有可落成正文的因果說明。
