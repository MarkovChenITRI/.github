...
applyTo: ".github/blueprint/**/*.md,.github/issues/**/*.md,blueprint/**/*.md"
---
...
`查核點定義表` 是跨 SOP 階段的凍結契約；`查核點簽核表` 是生命週期狀態欄。定義表描述要被做出來、如何做、如何驗；簽核表只記錄各角色在自身階段的責任狀態，不得反向改寫定義表。

| 欄位 | 所屬表格 | 欄位語意 |
|------|----------|----------|
| `Checkpoint ID` | 查核點定義表 / 查核點簽核表 | 跨 SOP 階段追蹤同一查核點的穩定 ID；簽核表不得新增無來源的 ID。 |
| `Item` | 查核點定義表 | 查核點短名稱，用來辨識要交付的成果類型。 |
| `完成條件` | 查核點定義表 | 成果被視為完成的客觀條件，不是規劃描述是否已寫完。 |
| `預期效益` | 查核點定義表 | 完成此查核點後對使用者、交付品質或後續工作帶來的效果。 |
| `執行方法 / 施工內容` | 查核點定義表 | SOP2 實際施工時要依循的方法包與變更內容。 |
| `驗證方式` | 查核點定義表 | SOP3 驗收該成果時要採用的檢查方式或證據類型。 |
| `證據位置` | 查核點定義表 | 施工或驗收證據可回查的位置；可先填預期位置，SOP2 / SOP3 再補成實際證據。 |
| `Planner` | 查核點簽核表 | SOP1 規劃凍結簽核；`confirmed` 只代表規劃契約已可交棒施工。 |
| `Executor` | 查核點簽核表 | SOP2 施工完成簽核；`confirmed` 只代表交付物已做出來且可追溯。 |
| `Validator` | 查核點簽核表 | SOP3 驗收簽核；`confirmed` 只代表實際交付物已通過驗收。 |
| `Notes` | 查核點簽核表 | 記錄偏差、阻擋、證據摘要、退回 owner 或下一步；不得用來覆蓋凍結契約。 |

SOP1 是規劃凍結，負責定義查核點契約、施工方法、完成條件、驗證方式與簽核欄位；不得把 SOP1 的查核表填寫解讀為成果已施工完成。SOP2 是實作施工，依 SOP1 凍結的施工藍圖把交付物做出來。SOP3 是交付驗收，驗證 SOP2 已做出的交付物是否符合 SOP1 凍結契約。若 SOP2 / SOP3 只找到規劃文件而沒有施工完成物，下一步是回到 SOP2 施工；不得回報為 SOP2 完成或 SOP3 通過。
...
3. 每份分項檔的查核點沿用固定欄位：`Checkpoint ID | Item | 完成條件 | 預期效益 | 執行方法 / 施工內容 | 驗證方式 | 證據位置`，並以 `Checkpoint ID | Planner | Executor | Validator | Notes` 追蹤生命週期簽核。
