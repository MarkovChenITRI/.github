...
2. `tests/operations/<flow-name>.md` 還沒有初稿時，交給 `plan` 技能的 `plan-usage-example.prompt.md` 建立；已有初稿時，直接沿用。
...
- 執行測試後有案例 fail。
- `.github/specs/presentation.md` 對應模組的段落被更新，測試還沒跟上。

### 判斷步驟

1. 開啟 `.github/specs/presentation.md`，找到對應模組目前的 What／How。
...
在 `tests/operations/<flow-name>.md` 更新步驟表與預期結果，在對話中輸入 `/operations-test` 重新執行並記錄結果。
