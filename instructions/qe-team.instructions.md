...
7. 圖表第一次被引用的位置出現在圖表本體之後。
8. 表格欄位包含理由、後果、建議處置、風險判讀或其他推論型內容。
9. 圖表後面的正文才第一次定義該圖表的用途，導致由上而下閱讀時先見圖表、後知其義。
10. 頁面移除圖表後，正文無法獨立成立，表示圖表被拿來補正文論證缺口。
...
當頁面屬概念說明、設計規格、維運機制或 prose-first 文件時，QE 應額外檢查：

1. 正文是否先完成主論證，圖表是否只作佐證。
2. 每張圖表前是否已有導入段，先講清楚支援哪個論點。
3. caption 是否只做命名，而沒有承擔正文論證。
4. 表格是否只保留穩定事實欄位，而非把推論塞進欄位。
5. 若某頁的主流程、主判斷或處置建議主要存在於圖表中，應退回要求 prose-first 重寫。
